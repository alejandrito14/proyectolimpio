...
class Clientes
{
   ...
   public function ClienteRfc()
	...
	public function ObtenerClientesTorneos($idtorneo)
	{

		$sql = "SELECT clientes.idcliente ,clientes.nombre,clientes.paterno,clientes.materno FROM clientes inner JOIN torneocliente ON clientes.idcliente=torneocliente.idcliente WHERE idtorneo='$idtorneo' GROUP BY clientes.idcliente ";
		$resp = $this->db->consulta($sql);
	...
	public function ObtenerClientesTorneos2($idtorneo)
	{

		$sql = "SELECT clientes.idcliente ,clientes.nombre,clientes.paterno,clientes.materno FROM clientes inner JOIN incripciontorneo ON clientes.idcliente=incripciontorneo.idcliente WHERE idtorneo='$idtorneo' ";
	...
	public function ObtenerJugados()
	{

		$sql = "SELECT *from juegocliente WHERE  idcliente='$this->idCliente' ";
		...
		if ($cont>0) {
			...
			while ($objeto=$this->db->fetch_object($resp)) {
					...
					$idjuego=$objeto->idjuego;


					$sql1="SELECT * FROM (SELECT  SUM(sets.marcador1) as marcador1 ,SUM(sets.marcador2) as marcador2,sets.idjuego,juego.idtorneo from  sets
						INNER JOIN juego ON juego.idjuego=sets.idjuego
						INNER JOIN juegocliente ON juego.idjuego=juegocliente.idjuego

						WHERE juego.idjuego='$idjuego'

						GROUP BY idjuego)AS TABLA";
						//echo $sql1;
					
					$resp1 = $this->db->consulta($sql1);
					$row=$this->db->fetch_assoc($resp1);
					...
					if ($row['marcador1']!=0 || $row['marcador2'] !=0) {
	...
	public function ObtenerJugadosGanados()
	{
		...
		if ($cont>0) {
			...
			while ($objeto=$this->db->fetch_object($resp)) {
				...
				if ($equipo==1) {
					...
					if ($row['marcador1'] > $row['marcador2']) {
	...
	public function ObtenerJugadosPerdidos()
	{
