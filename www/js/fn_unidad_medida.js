...
function Buscar_UnidadMedida(idmenumodulo)
{
	var id = $('#b_id').val();
	var nombre = $('#b_nombre').val();
	var estatus = $('#b_estatus').val();

	var datos = "idmedida=" + id + "&nombre=" + nombre + "&estatus=" + estatus + "&idmenumodulo=" + idmenumodulo;
	...
	$("#contenedor_empresas").html('<div align="center" class="mostrar"><img src="images/loader.gif" alt="" /><br />Cargando...</div>');	
		...
		setTimeout(function(){
				  $.ajax({
					url:'catalogos/unidadmedida/li_unidad_medida.php', //Url a donde la enviaremos
...
function GuardarMedida(form,regresar,donde,idmenumodulo)
{
	...
	{			
		...
		setTimeout(function(){
				  $.ajax({
					  url:'catalogos/unidadmedida/ga_unidad_medida.php', //Url a donde la enviaremos
